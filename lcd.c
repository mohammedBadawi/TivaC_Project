...
SYSCTL_RCGCGPIO_R|=(1<<0); //ENABLE CLK FOR PORTA 
while((SYSCTL_PRGPIO_R&(1<<0))==0){} 															//WAIT UNTIL CLK IS READY 
...
CONTROLPORT_CR_R |= 0X1C; 																				//ENABLE BITS
CONTROLPORT_AFSEL_R &=~(1<<RS);CONTROLPORT_AFSEL_R &=~(1<<RW);CONTROLPORT_AFSEL_R &=~(1<<ENB); //DISABLE ALT FUNCTION
}

void Data_Port_Init(void){
SYSCTL_RCGCGPIO_R|=(1<<1); //ENABLE CLK FOR PORTB 
while((SYSCTL_PRGPIO_R&(1<<1))==0){} //WAIT UNTIL CLK IS READY 
DATAPORT_DIR_R = 0XFF; //PORTB IS OUTPUT	
